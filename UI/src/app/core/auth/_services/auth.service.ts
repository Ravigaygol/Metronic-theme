...
/* const API_USERS_URL = 'http://localhost:8080/api/auth/'; */
const API_USERS_URL = 'http://localhost:8080/';
...
export class AuthService {
    ...
    login(username: string, password: string): Observable<User> {
        return this.http.post<User>(API_USERS_URL+'loginadmin', { username,password });
    }
    ...
    getUserByToken(): Observable<User> {
        ...
        return this.http.get<User>(API_USERS_URL+'loginadmin', { headers: httpHeaders });
    }

    register(user: User): Observable<any> {
        const httpHeaders = new HttpHeaders();
        httpHeaders.set('Content-Type', 'application/json');
        return this.http.post<User>(API_USERS_URL+'registeradmin1', user, { headers: httpHeaders })
            .pipe(
                map((res: User) => {
                    return res;
                }),
